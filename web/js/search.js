...
(function (window, document, undefined) {
	...
	let createResultsHTML = function (results) {
		let html = '<p>Found ' + results.length + ' matching items</p>';
	...
	let search = function (query) {
		...
		searchMeta.forEach(function (article) {
			if (reg.test(article.title)) return priority1.push(article);
			if (reg.test(article.author)) return priority2.push(article);
	...
	if (!form || !input || !resultList || !searchMeta) return;
